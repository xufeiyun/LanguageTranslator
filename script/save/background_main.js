...
var zhongwenMain = {
    ...
    _onTabSelect: function (tabId)
    {
        if ((this.enabled == 1))
        {
            console.error('should send message here');
            //chrome.tabs.sendRequest(tabId, { "type": "enable", "config": zhongwenMain.config });
        }
    },
    ...
    enable: function (tab)
    {
        ...
        console.error('should send message here');
        //chrome.tabs.sendRequest(tab.id, { "type": "enable", "config": zhongwenMain.config });
        zhongwenMain.enabled = 1;

        console.error('should send message here');
        //chrome.tabs.sendRequest(tab.id, { "type": "showPopup", "text": zhongwenMain.miniHelp, "isHelp": true });
