...
{
    chrome.runtime.onMessage.addListener(function (request, sender, sendResponse) { rcvmsg_cs_popup(request, sender, sendResponse); });
    
    // When a page first loads, checks to see if it should enable script
    chrome.runtime.sendMessage({ "type": "enable?" });
}

function rcvmsg_cs_popup(request, sender, sendResponse) 
{
    switch (request.type)
    {
        case 'enable':
